...
1) MERGE near-duplicates. Group sub-concepts that name the SAME assessable idea (e.g. plural/singular, reordered words, an abbreviation vs its expansion, or two phrasings of one step). Pick the clearest member name as the canonical name; carry the broad parent concept.
   ...
   ALSO fold a pure FRAMING sub-concept — "importance of X", "why X matters", "purpose/benefit of X", "X is useful/needed" — INTO the substantive X concept it frames (a question on X already carries its purpose; keeping both wastes a slot on a near-duplicate). Do NOT emit a standalone "importance of X" when "X" itself is a concept in the set. This is framing-only and does NOT license merging distinct parallel-family members.
