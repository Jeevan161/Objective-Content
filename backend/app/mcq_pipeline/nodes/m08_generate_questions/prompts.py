...
_FIX_PREFIX = register("gen.fix_prefix", (
    ...
    "- If the reviewer says a concrete detail is WRONG, not how it works here, or does "
    "not apply, treat that detail as ungrounded: REMOVE it and re-ground strictly in the "
    "COURSE MATERIAL — use the material's own value/wording when it gives one, and if it "
    "does not, rephrase the question CONCEPTUALLY (the steps and their order, what each "
    "achieves) so it no longer depends on that detail. Never swap in another invented "
    "value. (Example: the reviewer flags the port/URL as platform-specific — drop the "
    "invented `localhost:3000`, use the material's `DOMAIN_URL` if it gives one, else ask "
